...
const amplify_role_settings = (config) => {
  return `
# Amplify
...
resource "aws_iam_policy" "amplify_policy-${config.prog_name}" {
  name        = "amplify_policy-${config.prog_name}"
  description = "Start Amplify jobs"
  policy = data.aws_iam_policy_document.policy_amplify.json
}
resource "aws_iam_role_policy_attachment" "amplify_policy-${config.prog_name}" {
    role        = aws_iam_role.${config.prog_name}-role.name
    policy_arn  = aws_iam_policy.amplify_policy-${config.prog_name}.arn
}
`
}
export { role_settings, vpc_role_settings, secrets_manager_role_settings, invoke_lambda_role_settings, s3_role_settings, ses_role_settings, amplify_role_settings };