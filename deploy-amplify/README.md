# Rebuild the Amplify distribution daily

This lambda function triggers a daily rebuild of the art-bus-schedules Amplify distribution (production branch only). The fuction sends a request to the AWS Amplify API to start a new build for the production branch. This process will pull the latest GTFS data, information from Wordpress, and rebuild the static files for [ridetheart.com](https://ridetheart.com). The function is triggered by Bedrock daily at 3:00am.
